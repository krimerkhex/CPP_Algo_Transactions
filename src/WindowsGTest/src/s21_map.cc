...
Map::Node::Node(const maped_type value, const bool color)
    : value(value), red(color), left(&null), right(&null) {}

Map::Map() : size_(0), root_(&Node::null) {}
...
Map::Node *Map::newNode(const maped_type value) {
  size_++;
  std::cout << "Size Увеличен in newNode" << std::endl;
...
void Map::delNode(Node *node) {
  size_--;
  std::cout << "Size Уменьшен in delNode" << std::endl;
  delete node;
}
...
void Map::show(Node *node, int depth, char dir) {
  int n;
  if (node == &Node::null) return;
...
Map::check_code Map::check(Node *tree, int d, int &h) {
  ...
  if (left != &Node::null && tree->value.first < left->value.first ||
      right != &Node::null && tree->value.first > right->value.first)
    return error_struct;
  if (!tree->red) d++;
